...
from core.config import ModelConfig, resolve_config
from utils import load_checkpoint_adaptive

from training.losses import lm_loss, mse_loss, clip_contrastive_loss, diffusion_loss_fn, distill_loss, retrieval_accuracy
from data.datasets import (CocoCaptionDataset, CachedPairDataset, ImageCaptionCollate,
                       encode_captions, split_dataset, preprocess_image_path, preprocess_image_pil)
from training.optimizers import (build_new_module_optimizer, build_differential_optimizer,
...
def setup_joint(args, tokenizer, device):
    ...
    # Data
    from data.datasets import PadCollate
...
def setup_audio_clip(args, tokenizer, device):
    ...
    import csv
    from data.audio_synthetic import waveform_to_mel
    ...
    # Simple random split
    from data.datasets import split_dataset
...
def setup_base(args, tokenizer, device):
    ...
    from data.audio_synthetic import AudioDataset
    from data.video_synthetic import VideoDataset
    from data.datasets import PadCollate, interleave_loaders
